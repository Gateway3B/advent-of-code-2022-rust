use std::{fs::read_to_string, path::Path, io::Error};
use std::rc::Rc;
...
impl Builder for ListDirectoryBuilder {
  ...
  fn new() -> ListDirectoryBuilder {
    ListDirectoryBuilder { directory_items: Vec::new() }
  }
  ...
  fn build(self) -> Option<ListDirectory> {
    ...
    Some(ListDirectory {
      directory_items: self.directory_items.into_iter().filter_map(|item| { item }).collect()
    })
  }
}
...
impl Directory {
  fn child_directory(&mut self, name: &str) -> Option<Rc<RefCell<Directory>>> {
    self.directory_items.iter_mut().find_map(|item| { 
      match item {
...
fn lex_terminal_lines(lines: Vec<String>) -> Vec<TerminalCommand> {
  ...
  for line in lines.iter() {
      ...
      if let Some(builder) = current_command_builder {
       match builder {
         TerminalCommandBuilder::ChangeDirectoryBuilder(builder) => {
           match builder.build() {
             Some(command) => { terminal_commands.push(TerminalCommand::ChangeDirectory(command)) },
             None => { terminal_commands.push(TerminalCommand::Invalid(format!("{}", line!()).to_string())) }
           }
         },
         TerminalCommandBuilder::ListDirectoryBuilder(builder) => {
           match builder.build() {
             Some(command) => { terminal_commands.push(TerminalCommand::ListDirectory(command)) },
             None => { terminal_commands.push(TerminalCommand::Invalid(format!("{}", line!()).to_string())) }
           }
          }
        }
    ...
    match token {
      "ls" => {
        current_command_builder = Some(TerminalCommandBuilder::ListDirectoryBuilder(ListDirectory::builder()));
      },
      "cd" => {
        current_command_builder = Some(TerminalCommandBuilder::ChangeDirectoryBuilder(ChangeDirectory::builder()));
        ...
        match name {
          None => {
            current_command_builder = None;
            terminal_commands.push(TerminalCommand::Invalid(format!("{}", line!()).to_string()));
          },
          Some(name) => {
            if let Some(builder) = current_command_builder {
              if let TerminalCommandBuilder::ChangeDirectoryBuilder(mut builder) = builder {
        ...
        match name {
          None => {
            current_command_builder = if let Some(builder) = current_command_builder {
              Some(if let TerminalCommandBuilder::ListDirectoryBuilder(mut builder) = builder {
                builder.directory_items.push(None);
                TerminalCommandBuilder::ListDirectoryBuilder(builder)
              } else { builder })
            } else { current_command_builder }
          },
          Some(name) => {
            current_command_builder = if let Some(builder) = current_command_builder {
              Some(if let TerminalCommandBuilder::ListDirectoryBuilder(mut builder) = builder {
                builder.directory_items.push(Some(DirectoryItem::Directory(Rc::new(RefCell::new(Directory {
                  name: name.to_string(),
                  directory_items: Vec::new(),
                  size: None
                })))));
          ...
          Some(name_token) => {
            ...
            if let Some(name) = name_tokens.next() {
              ...
              if let Some(file) = file {
                current_command_builder = if let Some(builder) = current_command_builder {
                  Some(if let TerminalCommandBuilder::ListDirectoryBuilder(mut builder) = builder {
                    builder.directory_items.push(Some(DirectoryItem::File(file)));
                    TerminalCommandBuilder::ListDirectoryBuilder(builder)
                  } else { builder })
                } else { current_command_builder }
              }
    ...
    match command {
      TerminalCommand::ChangeDirectory(change_directory) => {
        match change_directory.name.as_str() {
          ...
          _ => {
            let child_directory = directory_stack.last().unwrap().borrow_mut().child_directory(change_directory.name.as_str());
      ...
      TerminalCommand::ListDirectory(list_directory) => {
        let directory_items = &mut directory_stack.last().unwrap().borrow_mut().directory_items;
...
fn get_directories_under_size(directories: Rc<RefCell<Directory>>, size: u32) -> Vec<Rc<RefCell<Directory>>> {
